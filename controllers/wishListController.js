...
exports.addToWishlist = async (req, res) => {
  try {
    ...
    let userWishlist = await Wishlist.findOne({ user: userId }).populate("products");
    if (!userWishlist) {
      userWishlist = new Wishlist({ user: userId, products: [] });
    }
    if (userWishlist.products.includes(productId)) {
      return res.status(400).json({ status: "failed", error: "Product already in wishlist" });
    }
...
exports.getWishlist = async (req, res) => {
  try {
    ...
    if (!userWishlist) {
      return res.status(404).json({ status: "failed", error: "Wishlist not found" });
    }
...
exports.removeFromWishlist = async (req, res) => {
  try {
    ...
    if (productIndex === -1) {
      return res.status(400).json({ status: "failed", error: "Product not in wishlist" });
    }
    userWishlist.products.splice(productIndex, 1);
    await userWishlist.save();

    res.status(200).json({ status: "success", message: "Product removed from wishlist successfully" });
  } catch (error) {
